...
//	std::cout<<wp.weights[0]<<" "<<wp.weights[1]<<" "<<wp.weights[2]<<std::endl;
	glm::vec3 rot_edge;
	if (!cross(ta) && !cross(tb) && !cross(tc)) { //if a triangle edge is not crossed
		...
		if(cross(ta)){
			...
			curr2 = wp.triangle.z;
			which_bary = 0;
			rot_edge = vertices[wp.triangle.x] - vertices[wp.triangle.z];
		}else if(cross(tb)){
			curr1 = wp.triangle.y;
			curr2 = wp.triangle.z;
			which_bary = 1;
			rot_edge = vertices[wp.triangle.y] - vertices[wp.triangle.z];
		}else{
			curr1 = wp.triangle.x;
			curr2 = wp.triangle.y;
			which_bary = 2;
			rot_edge = vertices[wp.triangle.x] - vertices[wp.triangle.y];
		}
		std::cout<<"fjdslkafjdsfjlkfjsaf"<<std::endl;
//		std::cout<<ta<<" "<<tb<<" "<<tc<<std::endl;

		glm::vec3 old_normal = world_normal(wp);
		auto next_v = next_vertex.find(glm::uvec2(curr2, curr1));
		...
		if(next_v != next_vertex.end()){
			...
			glm::vec3 cp = closest_pt(a, b, c, proj_step);
			wp.weights = get_barycentric(a, b, c, cp);
			wp.weights[which_bary] = 0.01;
			//step gets rotated over the edge

			glm::vec3 new_normal = world_normal(wp);
			if(old_normal!=new_normal){
				std::cout<<old_normal[0]<<" "<<old_normal[1]<<" "<<old_normal[2]<<std::endl;
				std::cout<<new_normal[0]<<" "<<new_normal[1]<<" "<<new_normal[2]<<std::endl;
				...
				float angle = std::acos(numer)*180.0f/M_PI;
				glm::quat rotationQuat = glm::angleAxis(angle, rot_edge);
